...
import com.daml.metrics.{DatabaseMetrics, Metrics, SpanAttribute, Timed}
import com.daml.platform.ApiOffset
import com.daml.platform.store.DbType
import com.daml.platform.store.SimpleSqlAsVectorOf.SimpleSqlAsVectorOf
import com.daml.platform.store.dao.{DbDispatcher, PaginatingAsyncStream}
import io.opentelemetry.OpenTelemetry
import io.opentelemetry.trace.{DefaultSpan, Span}
...
private[dao] final class TransactionsReader(
  ...
  private val tracer = OpenTelemetry.getTracer("transactions-reader")
  ...
  def getFlatTransactions(
    ...
    groupContiguous(events)(by = _.transactionId)
      ...
      .map { case (offset, response) =>
        response.transactions.foreach(txn => txnSpan.addEvent(daml.metrics.Event("transaction-received", TraceIdentifiers.fromTransaction(txn))))
        (offset, response)
      }
  }
  ...
  private def streamEvents[A: Ordering, E](
      ...
      txnSpan: Span = DefaultSpan.getInvalid
  )(range: EventsRange[A])(
      implicit loggingContext: LoggingContext,
