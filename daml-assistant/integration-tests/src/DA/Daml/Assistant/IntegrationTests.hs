    ...
    let startProc =
            (shell $
             unwords
                ...
                {std_in = CreatePipe, cwd = Just tmpDir}
    withEnv [("DAML_PROJECT", Just projDir)] $ do
...
tests tmpDir =
    withSdkResource $ \_ ->
        testGroup
            ...
            , withResource (damlStart tmpDir) (terminateProcess . startPh) damlStartTests
...
damlStartTests getDamlStart =
    testGroup
        ...
        [ testCase "sandbox and json-api come up" $ do
          ...
          -- this tests a couple of things at once:
          --  - the option --sandbox-port=0 was picked up and a port-file written
          --  - sandbox-options where picked up
          --  - daml start worked with a relative project path
        ...
        , testCase "hot-reload" $ do
              ...
              withCurrentDirectory projDir $ do
                  ...
                  queryResponse <- httpLbs queryRequest manager
                  statusCode (responseStatus queryResponse) @?= 200
                  preview (key "result" . nth 0 . key "payload" . key "newFieldName") (responseBody queryResponse) @?=
                      Just "Alice"
